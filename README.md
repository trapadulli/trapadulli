# Stephan van Woezik

Software Engineer | .NET | Cloud Systems | Quantitative Platforms | Music Tech

---

## 🚀 About Me

I’m a software engineer with 11+ years of experience building systems across financial platforms, cloud infrastructure, and full-stack applications. I focus on ownership, reliability, and delivering production systems that actually hold up under real-world use.

My work spans backend services, APIs, data modeling, and frontend systems, with deep experience in .NET, Azure, and SQL Server. I’ve built and operated platforms for financial analytics, forecasting, and operational decision-making.

I also build tools at the intersection of **music theory, AI, and audio processing**, including systems for pitch recognition, solfège analysis, and real-time feedback.

- Tech patent holder  
- B.S. Computer Science (cum laude), minor in Business  
- 10+ years building and shipping production systems  

---

## 🧠 Focus Areas

- Distributed systems & backend architecture  
- Financial modeling & trading systems  
- Cloud-native applications (Azure)  
- Audio + AI (pitch detection, solfège, inference systems)  
- Full-stack product development  

---

## ⚙️ Tech Stack

**Backend:**  
C# • .NET • Web API • SQL Server • LINQ  

**Cloud & DevOps:**  
Azure • CI/CD • Azure DevOps • Cloud Architecture  

**Frontend / Mobile:**  
React • React Native • TypeScript  

**AI / Data:**  
Python • PyTorch • ONNX • Audio Processing  

---

## 🌟 Projects

### [Musilinda](https://github.com/Musilinda)
iOS Solfège-based music education platform — interactive app with real-time AI pitch recognition and a PyTorch/Whisper ML pipeline for syllable classification.
`React · TypeScript · Node.js · PostgreSQL · ONNX · Capacitor`

### [MapStreamKit](https://github.com/trapadulli/MapStreamKit)
Infrastructure as code - event-driven ingestion and control-plane that pulls external API data, normalizes, and exposes it through a read API layer.
`Event Hub · Serverless Functions · NoSQL · Object Storage · GraphQL (DAB) · Infrastructure as Code`

### [Capacitor AddRev Kit](https://github.com/trapadulli/Capacitor-AddRev-Kit)
Shell for wrapping a web application inside a WebView for iOS (and optionally Android). 
`Capacitor · TypeScript · iOS (Swift) · WebView · AdMob · RevenueCat`

### [SimTrade](https://github.com/SimTrade)
Quantitative algorithmic trading platform — equity data ingest pipeline and backtester/live-trader running against brokerage APIs.
`Node.js · Azure · MongoDB · Alpaca API`

### [StoryLint](https://github.com/StoryLint)
A Chrome extension that converts confusing product stories into meaningful acceptance criteria.
`LLM integrations · Javascript · Python · FastAPI · Docker`

---

## 🧩 What I Actually Do

- Own systems end-to-end (design → build → deploy → support)  
- Turn messy requirements into working software  
- Modernize legacy systems without breaking production  
- Build things that are fast, stable, and maintainable  

---

## 🏆 Highlights

- 11+ years professional software engineering experience  
- Built systems used in financial, SaaS, and enterprise environments  
- Experience across startups and Fortune 500 companies  
- Strong focus on production reliability and system ownership  

---

## ⚡ Interests

- Music theory (Lydian Chromatic Concept, tonal gravity)  
- AI applied to audio and learning systems  
- Financial systems and trading models  
- Building products that actually teach people something  

---

## 📫 Connect

- LinkedIn: https://linkedin.com/in/stephanvanwoezik
- GitHub: https://github.com/trapadulli

---

> Build things that work. Then make them better.

# Stephan Vanwoezik

Software engineer building systems across music education, algorithmic trading, and developer tooling, with a focus on AI deployments, audio deep learning technology, and event-driven architectures.

